...
class Shared2FCInstanceMILHead(BBoxHead):
    ...
    def loss_mil(self,
        ...
        if stage < 1:  # TODO: 这里没有用到reg_box 和 gt_boxes
            if cls_score is not None:
                avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
                if cls_score.numel() > 0:
                    label_valid = proposals_valid_list
                    cls_score = cls_score.softmax(dim=-1)  # cls score激活后输入mil loss
                    # TODO: 这里将gt拆分,观察不同尺度的roi的cls_score和ins_score
                    # gt_true_bboxes_bag = gt_true_bboxes.reshape(cls_score.size(0), -1, gt_true_bboxes.size(-1))
                    ins_prob_tmp = ins_score.softmax(dim=1)
                    ins_prob_tmp = F.normalize(ins_prob_tmp, dim=1, p=1)
                    # # gt box w h
                    # prob = cls_score * ins_prob_tmp
                    # cls_0 = cls_score[0]
                    # ins_0 = ins_prob_tmp[0]
                    # score_0 = prob[0]
                    # gt_bbox_0 = gt_true_bboxes[0]
                    # if gt_bbox_0[2] - gt_bbox_0[0] > 100:
                    #     label_0 = labels[0]
                    #     prob_score_0 = score_0[:,int(label_0)]
                    #     cls_score_0 = cls_0[:,int(label_0)]
                    #     ins_score_0 = ins_0[:,int(label_0)]
            ...
            if mode == 'cluster cam':
                if cls_score is not None:
                    cls_score = cls_score.sigmoid()  # refine阶段使用sigmoid
                    num_sample = cls_score.shape[0]
