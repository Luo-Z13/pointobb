...
class MILLoss(nn.Module):
    ...
    def forward(self, bag_cls_prob, bag_ins_outs, labels, valid, weight=None):
        ...
        B, N, C = bag_cls_prob.shape  # B为gt_num, N为prposals_num_per_gt, C为class_num+1
        ...
        label_valid = label_weights.reshape(-1).bool()
        acc = accuracy(prob[..., 0][label_valid], labels[label_valid])
        ...
        if self.loss_type == 'gfocal_loss':
            ...
            # TODO: 添加,否则当prob=0(rot的vaild被赋予False时),loss会出现100这样的大数
