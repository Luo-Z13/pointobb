...
class Resize:
    ...
    def __init__(self,
        ...
        if keep_ratio is True:
            self.pad_transform = Pad(size=img_scale)
...
class RandomFlip:
    ...
    def bbox_flip(self, bboxes, img_shape, direction):
        ...
        if bboxes.shape[-1] % 4 == 0:
            ...
            assert ((bboxes.shape[-1] % 4 == 0) or (bboxes.shape[-1] % 5 == 0))
...
class RandomCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('bbox_fields', []):
            if key == 'gt_true_bboxes' and results[key].shape[-1]==5:
